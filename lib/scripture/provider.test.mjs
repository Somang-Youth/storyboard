...
test('strips duplicated leading verse numbers from verse text', async () => {
  const { parseBskoreaChapterHtml } = await loadProviderModule();
  const verses = parseBskoreaChapterHtml(
    `
      <html><body>
        <div class="leftCont">
          <span><span class="number">17&nbsp;&nbsp;&nbsp;</span>17 유대인이라 불리는 네가 율법을 의지하며</span>
        </div>
      </body></html>
    `,
    john,
...
test('rejects non-positive and reversed source verse ranges', async () => {
  const { parseBskoreaChapterHtml } = await loadProviderModule();

  for (const range of ['0-2', '3-2']) {
    const html = `
      <div class="leftCont">
        <span><span class="number">${range}&nbsp;&nbsp;&nbsp;</span>잘못된 범위 본문</span>
        <span><span class="number">4&nbsp;&nbsp;&nbsp;</span>정상 본문</span>
      </div>
    `;

    assert.throws(
      () => parseBskoreaChapterHtml(html, john, 3),
      /절 범위가 올바르지 않습니다/,
    );
  }
});
